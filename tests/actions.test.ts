...
describe('Git action outcomes', () => {
  ...
  describe('undo', () => {
    const head = async () => (await git(repo, ['rev-parse', 'HEAD'])).trim()
    async function undo(result: Awaited<ReturnType<typeof runAction>>) {
      if (!result.undo) throw new Error('Expected an undo step')
      const request = decodeActionRequest({ repo, ...result.undo })
      return runAction(request.repo, request.action, request.args)
    }

    it('reverts a hard reset including staged and unstaged changes', async () => {
      ...
      await write('untracked.txt', 'untracked\n')
      const result = await runAction(repo, 'reset', { hash: 'HEAD~1', mode: 'hard' })
      expect(await read('file.txt')).toBe('one\n')
      await undo(result)
      expect(await head()).toBe(tip)
      expect(await git(repo, ['show', ':file.txt'])).toBe('staged\n')
      expect(await read('file.txt')).toBe('unstaged\n')
      expect(await read('untracked.txt')).toBe('untracked\n')
    })

    it('refuses to undo a reset after switching branches', async () => {
      await write('file.txt', 'one\n')
      await commit('one')
      await write('file.txt', 'two\n')
      await commit('two')
      const result = await runAction(repo, 'reset', { hash: 'HEAD~1', mode: 'soft' })
      await git(repo, ['checkout', '-q', '-b', 'other'])
      await expect(undo(result)).rejects.toThrow('main is no longer the checked out branch')
    })

    it('restores a dropped commit', async () => {
      await write('file.txt', 'one\n')
      await commit('one')
      await write('dropped.txt', 'dropped\n')
      await commit('dropped')
      await write('file.txt', 'three\n')
      await commit('three')
      const tip = await head()
      const result = await runAction(repo, 'dropCommit', { hash: 'HEAD~1' })
      await expect(read('dropped.txt')).rejects.toMatchObject({ code: 'ENOENT' })
      await undo(result)
      expect(await head()).toBe(tip)
      expect(await read('dropped.txt')).toBe('dropped\n')
    })

    it('recreates a deleted branch with its upstream', async () => {
      await write('file.txt', 'one\n')
      await commit('one')
      await git(repo, ['branch', 'feature'])
      await git(repo, ['branch', '--set-upstream-to=main', 'feature'])
      const hash = (await git(repo, ['rev-parse', 'feature'])).trim()
      const result = await runAction(repo, 'deleteBranch', { name: 'feature' })
      await undo(result)
      expect((await git(repo, ['rev-parse', 'feature'])).trim()).toBe(hash)
      expect((await git(repo, ['rev-parse', '--abbrev-ref', 'feature@{u}'])).trim()).toBe('main')
    })

    it('restores discarded files and discard-all changes from a snapshot', async () => {
      await write('a.txt', 'a\n')
      await write('b.txt', 'b\n')
      await commit('base')
      await write('a.txt', 'a staged\n')
      await git(repo, ['add', 'a.txt'])
      await write('b.txt', 'b changed\n')
      await write('new.txt', 'new\n')
      const one = await runAction(repo, 'discard', { paths: ['a.txt', 'new.txt'] })
      expect(await read('a.txt')).toBe('a\n')
      expect(await read('b.txt')).toBe('b changed\n')
      await expect(read('new.txt')).rejects.toMatchObject({ code: 'ENOENT' })
      await undo(one)
      expect(await git(repo, ['show', ':a.txt'])).toBe('a staged\n')
      expect(await read('new.txt')).toBe('new\n')
      const all = await runAction(repo, 'discardAll', { includeUntracked: true })
      expect(await git(repo, ['status', '--porcelain'])).toBe('')
      expect(await git(repo, ['stash', 'list'])).toBe('')
      await undo(all)
      expect(await read('a.txt')).toBe('a staged\n')
      expect(await read('b.txt')).toBe('b changed\n')
      expect(await read('new.txt')).toBe('new\n')
    })
  })
